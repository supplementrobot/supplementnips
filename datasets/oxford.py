...
class OxfordDataset(Dataset):
    ...
    def __getitem__(self, ndx):
        ...
        if self.use_cloud:
            ...
            coords, clouds = ME.utils.sparse_quantize(coordinates=query_pc, features=query_pc, quantization_size=1) # boreas quantization_size=1
        ...
        if args.dataset in ['oxford','oxfordadafusion']:
            ...
            if args.sph_cloud_fe is not None:
                file_pathname_sph = file_pathname.replace('lidar_1_4096_interval10','lidar_sph100_interval10')
                file_pathname_sph = file_pathname_sph.replace('.npy','.png')
                assert os.path.exists(file_pathname_sph)
                sph_cloud = plt.imread(file_pathname_sph)[:,:,:3]
                sph_transform = TVT.Compose([
                    TVT.ToTensor(),
                    TVT.Normalize(mean=0.5, std=1)
                ])
                sph_cloud = sph_transform(sph_cloud)
                result['sph_cloud'] = sph_cloud
