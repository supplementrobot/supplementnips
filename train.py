...
def do_train(dataloaders, params: MinkLocParams, debug=False):
    ...
    recall_top1p_best = [0, 0]
    recall_top1_best = [0, 0]
    ...
    for epoch in range(args.epochs):
        ...
        for i_batch, batch_dict in tqdm(enumerate(dataloaders['train'])):
            ...
            # coord = batch_dict['coords'][:2000,1:]
            # coord_feat = batch_dict['features_clouds'][:2000,1:]
            # colors = np.ones([ len(coord)*2 ])
            # colors[:len(coord)] *= 0
            # pc0 = coord.cpu().detach().numpy()
            # pc1 = coord_feat.cpu().detach().numpy()*100
            # pc = np.concatenate([pc0, pc1])
            # viz_lidar_mayavi(pc, label=colors)
        ...
        #     torch.save(model.state_dict(), os.path.join(args.models_dir, f'ep{epoch}.pth'))
        ...
        if recall_top1p > recall_top1p_best[0]:
            recall_top1p_best = [recall_top1p, epoch]
            if args.save_weights:
                torch.save(model.state_dict(), os.path.join(args.models_dir, f'recall_top1p_best_ep{epoch}_{recall_top1p.round(2)}.pth'))


        if recall_top1 > recall_top1_best[0]:
            recall_top1_best = [recall_top1, epoch]
            if args.save_weights:
                torch.save(model.state_dict(), os.path.join(args.models_dir, f'recall_top1_best_ep{epoch}_{recall_top1.round(2)}.pth'))
        ...
        txt.append(f'recall_top1p_best                {recall_top1p_best[0]:.2f}\t  epoch {recall_top1p_best[1]}')
        txt.append(f'recall_top1_best                 {recall_top1_best[0]:.2f}\t  epoch {recall_top1_best[1]}')
